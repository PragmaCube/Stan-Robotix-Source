...
void SubAlgaePivot::manualAlgaePivot(){
    ...
    mAlgaePivotMotor->SetVoltage(-units::volt_t(AlgaeIntakeConstants::kG) * cos((mAlgaePivotMotor->GetEncoder().GetPosition() + 4.54766) / 64 * std::numbers::pi) + units::voltage::volt_t(frc::SmartDashboard::GetNumber("armVoltage", 0.2274)));  
    ...
    std::cout << mAlgaePivotMotor->GetEncoder().GetVelocity() / 64.0 * 2.0 / 60.0 * std::numbers::pi << std::endl;
